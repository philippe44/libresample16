...
int writeFilter(HWORD Imp[], HWORD ImpD[], UHWORD LpScl, UHWORD Nmult, 
		UHWORD Nwing)
{
   char fname[30], file[30];
   FILE *fp;
   int i;

   sprintf(fname, "F%dT%d", Nmult, Nhc);
   sprintf(file, "%s.h", fname);
   fp = fopen(file, "w");
      if (!fp)
      return(1);
   fprintf(fp, "#define %s_SCALE %d\n", fname, LpScl);
   fprintf(fp, "#define %s_NWING %d\n", fname, Nwing);
   fprintf(fp, "#define %s_NMULT %d\n", fname, Nmult);
   fprintf(fp, "static const HWORD %s_IMP[] = {\n", fname);
   for (i=0; i<Nwing; i++)   /* Put array of 16-bit filter coefficients */
      fprintf(fp, "%d,\n", Imp[i]);
   fprintf(fp, "};\n");	  
   fprintf(fp, "static const HWORD %s_IMPD[] = {\n", fname);
...
WORD FilterUD( const HWORD Imp[], const HWORD ImpD[],
		     UHWORD Nwing, BOOL Interp,
